...
function renderTasks(){
  ...
  state.tasks.forEach((t, idx)=>{
    ...
    li.setAttribute('data-id', t.id || idx);
...
function setupEventListeners() {
  ...
  $('#focusMins').addEventListener('change', e=>{ 
    state.durations.focus = Math.max(1, +e.target.value)*60; 
    if(state.mode==='focus') resetTimer(); 
    persist();
  });
  $('#shortMins').addEventListener('change', e=>{ 
    state.durations.short = Math.max(1, +e.target.value)*60; 
    if(state.mode==='short') resetTimer(); 
    persist();
  });
  $('#longMins').addEventListener('change', e=>{ 
    state.durations.long = Math.max(1, +e.target.value)*60; 
    if(state.mode==='long') resetTimer(); 
    persist();
  });
  $('#cyclesToLong').addEventListener('change', e=>{ 
    state.cyclesToLong = +e.target.value; 
    updateStats(); 
    persist();
  });
  $('#autoStart').addEventListener('change', e=>{ 
    state.autoStart = e.target.value==='on'; 
    persist(); 
  });
